...
int main(void) {
	...
	cv::VideoCapture capWebcamA(1);            // Declare 2 VideoCapture objects associate them to the 2 webcams used
	cv::VideoCapture capWebcamB(1);            //Note: The numbers in the brackets might need to be changed in order to get the right Webcam
	...
	while (capWebcamA.isOpened() && chCheckForEscKey != 27 && capWebcamB.isOpened()) { //Check that Webcam is still open or if ESC key is pressed
		...
		cv::imshow("PRODUCT-B", BGSubtractionB);               //show the image with the Filter applied
