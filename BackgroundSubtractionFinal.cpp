// The BackgroundSubtractionFinal program finalises on the tested "BackgroundSubtraction" program as well as implementing user input to customise the background taken
...
int main(void) {
	...
	cv::VideoCapture capWebcamA(0);            // Declare 2 VideoCapture objects associate them to the 2 webcams used
    ...
    while (capWebcamA.isOpened() && chCheckForEscKey != 27 && capWebcamB.isOpened()) { //Check that Webcam is still open or if ESC key is pressed
		...
		while (getOut == false) {               //While we need to continue taking background samples
			capWebcamA.read(CamFeedA);     //Get the next fram from respective Camera feeds
			capWebcamB.read(CamFeedB);
			
			std::cout << "please press 1 to capture background\n"; //Message to user
			std::cin >> capture;

			if (capture == 1) {               //if "1" is pressed, get the current camera feed and set as background
				capWebcamA.read(BackgroundA);
				capWebcamB.read(BackgroundB);
				capture = 0;
			}
			
			std::cout << "is this good background?\n"; //ask user to see if background capture is good
			
			cv::imshow("a", BackgroundA);// show background images to user
			cv::imshow("b", BackgroundB);
			cv::waitKey(1);
			
			std::cout << "1 to take backgound again, 2 to start background subtraction\n";  //options for user
			std::cin >> capture;

			if (capture == 1) {  //If 1 is pressed, redo the loop, else, escape loop
				getOut = false;
				capture = 0;
			}
			else {
		...
		cv::GaussianBlur(CamFeedCopyA, CamFeedCopyA, cv::Size(21,21), 15);   //Add Gaussian Blur to "CamFeedCopyA" to reduce noise.
		cv::GaussianBlur(CamFeedCopyB, CamFeedCopyB, cv::Size(21, 21), 15);   //Add Gaussian Blur to "CamFeedCopyB" to reduce noise.
		cv::GaussianBlur(BackgroundACopy, BackgroundACopy, cv::Size(21, 21), 15);   //Add Gaussian Blur to "BackgroundACopy" to reduce noise.
		cv::GaussianBlur(BackgroundBCopy, BackgroundBCopy, cv::Size(21, 21), 15);   //Add Gaussian Blur to "BackgroundBCopy" to reduce noise.
		...
		cv::threshold(ImgDifferenceA, ImgThreshA, 25, 255, CV_ADAPTIVE_THRESH_MEAN_C);   //Threshold the "ImgDifferenceA", save to "ImgThreshA"
		cv::threshold(ImgDifferenceB, ImgThreshB, 25, 255, CV_ADAPTIVE_THRESH_MEAN_C);   //Threshold the "ImgDifferenceB", save to "ImgThreshB"

		cv::Mat structuringElement = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(15,15)); //Declare a Structuring Element in order
		...
		cv::findContours(ImgThreshCopyA, ContoursA, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);  //Find contours of "ImgThreshCopyA"
		cv::drawContours(ImgContoursA, ContoursA, -1, SCALAR_WHITE, -1);  //Draw contours onto the "ImgContoursA" Matrix

		cv::findContours(ImgThreshCopyB, ContoursB, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);  //Find contours of "ImgThreshCopyB"
		cv::findContours(ImgThreshCopyB, ContoursB, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);  //Find contours of "ImgThreshCopyB"
		...
		cv::imshow("PRODUCT-A", BGSubtractionA);               //show the image with the filters applied
		...
		cv::imshow("PRODUCT-B", BGSubtractionB);  
		
		cv::namedWindow("a", CV_WINDOW_NORMAL);      // note: you can use CV_WINDOW_NORMAL which allows resizing the window
		cv::namedWindow("b", CV_WINDOW_NORMAL);       // or CV_WINDOW_AUTOSIZE for a fixed size window matching the resolution of the image
															  // CV_WINDOW_AUTOSIZE is the default show windows
		cv::imshow("a", BackgroundA);                   // show the original webcam feed (first)
		cv::imshow("b", BackgroundB);               //show the image with the filters applied
//show the image with the Filter applied
